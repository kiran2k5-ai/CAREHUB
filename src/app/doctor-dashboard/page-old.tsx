  ...
  const loadDashboardData = async (doctorId: string) => {
    try {
      ...
      if (appointmentsRes.ok) {
        ...
        const todayAppts = appointmentsList.filter((apt: any) => apt.date === today);
        const pendingAppts = appointmentsList.filter((apt: any) => apt.status === 'pending');
        
        setStats({
          todayAppointments: todayAppts.length,
          pendingAppointments: pendingAppts.length,
          totalPatients: appointmentsList.length,
          monthlyEarnings: appointmentsList.reduce((sum: number, apt: any) => sum + (apt.consultationFee || 500), 0),
