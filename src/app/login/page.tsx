...
export default function LoginPage() {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // Direct login - save auth data and redirect
      if (data.userData && data.redirectUrl) {
        setAuthData(data.userData, data.userType, data.authToken);
        
        // Small delay to ensure localStorage is written
        await new Promise(resolve => setTimeout(resolve, 100));
        
        console.log(`Redirecting ${data.userType} to:`, data.redirectUrl);
        ...
        <div className="w-full max-w-sm mx-auto">
          <div className="text-center mb-8">
            ...
            <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-xs text-blue-800 font-medium mb-2">Direct Login (No OTP):</p>
              <div className="text-xs text-blue-700 space-y-1">
                <p><strong>Patient:</strong> Any number (e.g., 9042222856)</p>
                <p><strong>Doctor:</strong> Numbers starting with 987654321X or ending with 0</p>
                <p className="text-blue-600 mt-1">• Auto-redirects to appropriate dashboard</p>
              </div>
            </div>
          </div>
