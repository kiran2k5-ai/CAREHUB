...
export default function DoctorDetailPage() {
  ...
  // Get user ID from authentication instead of hardcoded value
  const authData = getAuthData();
  const userId = authData?.userData?.id || null;
  
  // Add the localStorage-enabled appointments hook with real user ID
  const { bookAppointment } = useAppointments(userId);
  ...
  useEffect(() => {
    ...
    const initializeData = async () => {
      ...
      try {
        ...
        // Ensure patient has a proper ID in the format expected by the system
        let patientId = patient.id;
        if (!patientId || patientId === 'undefined') {
          // Convert phone number to proper patient ID format
          const phoneNumber = patient.phone ? patient.phone.replace(/[^0-9]/g, '') : '9042222856';
          patientId = `patient_${phoneNumber}`;
        }
        
        // Update patient data with proper ID
        const updatedPatient = {
          ...patient,
          id: patientId
        };
        
        console.log('Patient data loaded with proper ID:', updatedPatient);
    ...
    const fetchDoctor = async () => {
      try {
        ...
        const apiUrl = `/api/doctors/${doctorId}`;
        ...
        if (response.ok) {
          ...
          if (result.success && result.data) {
            ...
            console.log('✅ Doctor data set:', result.data.name);
          } else {
            console.error('❌ Failed to fetch doctor data - Invalid result structure:', result);
          }
        } else {
          const errorText = await response.text();
          console.error('❌ Failed to fetch doctor data - HTTP error:', {
        ...
        console.error('❌ Error fetching doctor - Network/Parse error:', error);
  ...
  const handleBookAppointment = async () => {
    ...
    if (!currentPatientData || !currentPatientData.id) {
      ...
      if (userType === 'patient' && userData) {
        try {
          ...
          // Use phone number directly or UUID if available
          if (!parsedData.id && parsedData.phone) {
            parsedData.id = parsedData.phone.replace(/[^0-9]/g, ''); // Just the phone number
          }
      ...
      if (!currentPatientData || !currentPatientData.id) {
        const fallbackPhone = '9042222856';
        currentPatientData = {
          id: fallbackPhone, // Use phone number directly
          name: 'Demo Patient',
          email: 'patient@demo.com',
          phone: fallbackPhone
        };
    ...
    try {
      ...
      const appointmentData = {
        ...
        doctorId: doctor.id, // Keep original doctor ID, let API handle mapping
      ...
      console.log('Doctor ID being sent:', appointmentData.doctorId);
      ...
      if (response.ok && result.success) {
        // Set a flag to indicate that dashboards should refresh
        localStorage.setItem('appointmentJustBooked', 'true');
        localStorage.setItem('lastAppointmentBooking', Date.now().toString());
        
        // Show success message and redirect
        alert('Appointment booked successfully! Your appointment will appear in your dashboard shortly.');
