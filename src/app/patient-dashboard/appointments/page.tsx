...
export default function PatientAppointments() {
  ...
  const initializeAppointments = async () => {
    ...
    // For development/demo purposes, use the known patient ID
    const demoPatient = {
    ...
    await loadAppointments(demoPatient.id);
  };

  const loadAppointments = async (patientId: string) => {
    try {
      // Use the passed patientId parameter instead of hardcoded UUID
      console.log('Loading appointments for patient:', patientId);
      
      // Add timestamp to break any caching issues
      const timestamp = new Date().getTime();
      
      // Try relative URL first, fallback to absolute URL if needed
      const baseUrl = typeof window !== 'undefined' ? window.location.origin : 'http://localhost:3000';
      const apiUrl = `/api/appointments?patientId=${patientId}&t=${timestamp}`;
      ...
      if (response.ok) {
        ...
        console.warn('Failed to load appointments from database');
        setAppointments([]);
      }
    } catch (error) {
      console.error('Error loading appointments:', error);
      // No mock data fallback - only real database data
      setAppointments([]);
    } finally {
      setLoading(false);
    }
  ...
  const filteredAppointments = Array.isArray(appointments) ? appointments.filter(appointment => {
    console.log('🔍 Filtering appointment:', appointment.id, 'Status:', appointment.status);
    const status = appointment.status as string; // Cast to string to handle different status formats
