...
export default function MedicalRecordsPage() {
  ...
  const initializePage = async () => {
    ...
    try {
      ...
      // Use the correct test patient UUID to match database records
      const correctPatientId = '7f8e3607-a428-4241-aa3f-10a071f584fa';
      console.log('🔍 Patient records page using verified UUID:', correctPatientId);
      
      setPatientId(correctPatientId);
      await loadMedicalRecords(correctPatientId);
    } catch (error) {
      console.error('Error initializing records page:', error);
      // Fallback to correct patient ID
      const correctPatientId = '7f8e3607-a428-4241-aa3f-10a071f584fa';
      setPatientId(correctPatientId);
      await loadMedicalRecords(correctPatientId);
      setLoading(false);
  ...
  const loadMedicalRecords = async (patientId: string) => {
    try {
      ...
      // Fetch real medical records from the API
      const medicalRecordsRes = await fetch(`/api/medical-records?patientId=${patientId}`, {
        cache: 'no-store',
        headers: {
      ...
      let realMedicalRecords = [];
      if (medicalRecordsRes.ok) {
        try {
          const medicalData = await medicalRecordsRes.json();
          console.log('📋 Medical records from API:', medicalData);
          
          if (medicalData.success && Array.isArray(medicalData.data)) {
            realMedicalRecords = medicalData.data;
            console.log(`✅ Found ${realMedicalRecords.length} real medical records`);
          }
        } catch (parseError) {
          console.error('❌ Error parsing medical records:', parseError);
        }
      }

      // Convert real medical records to appointment format for display
      const medicalRecordAppointments = realMedicalRecords.map((record: any) => {
        ...
        return {
          ...
          doctorName: 'Dr. Unknown', // We'll need to fetch doctor names separately or add to API
          doctorSpecialization: 'General Medicine',
          date: record.visit_date || record.date,
          time: record.visit_time || record.time || '00:00',
          status: 'completed',
          consultationFee: 0,
          consultationType: 'in-person',
          reason: 'Medical Record Entry',
          diagnosis: record.diagnosis || 'No diagnosis available',
          prescription: (record.prescriptions && record.prescriptions.length > 0) ? 'Available' : 'Not Required',
          prescriptionData: {
            medications: record.prescriptions || [],
            instructions: record.treatment_plan || 'Follow general health guidelines',
            restrictions: 'Consult doctor before making any changes'
          },
          labReports: record.lab_reports || [],
          notes: record.examination_notes || '',
      ...
      console.log('📊 Converted medical records to appointments:', medicalRecordAppointments);
      setCompletedAppointments(medicalRecordAppointments);
      
      // Generate medical records from real data
      const generatedRecords = medicalRecordAppointments.map((appointment: Appointment) => 
  ...
  const generatePrescriptionDetails = (appointment: Appointment) => {
    // Handle prescription data from the appointment record
    const prescriptionText = appointment.prescription;
    if (prescriptionText) {
      try {
        ...
        if (parsed.medications && parsed.medications.length > 0) {
          return {
            medications: parsed.medications.map((med: any) => ({
              name: med.medicationName || med.name || 'Unknown Medication',
              dosage: med.dosage || 'As prescribed',
              frequency: med.frequency || 'As directed',
              duration: med.duration || '30 days',
              instructions: med.instructions || 'Take as directed by doctor'
            })),
            instructions: parsed.instructions || 'Follow doctor\'s advice',
            restrictions: parsed.restrictions || 'Consult doctor before making any changes'
          };
        }
      } catch (e) {
        // If parsing fails, treat as plain text prescription
        return {
          medications: [],
    ...
    // Fallback to specialty-based generation for older records
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="px-3 sm:px-4 md:px-6 py-4 sm:py-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
              <button
                onClick={() => router.back()}
                className="p-1.5 sm:p-2 text-white hover:text-green-100 hover:bg-white/20 rounded-lg flex-shrink-0"
              >
                <ChevronLeftIcon className="w-5 h-5 sm:w-6 sm:h-6" />
              </button>
              <h1 className="text-lg sm:text-xl font-semibold text-white truncate">Medical Records</h1>
            </div>
            <button
              onClick={() => setShowFilters(!showFilters)}
              className="p-1.5 sm:p-2 text-white hover:text-green-100 hover:bg-white/20 rounded-lg self-end sm:self-auto flex-shrink-0"
            >
      ...
      <div className="bg-white border-b">
        <div className="px-3 sm:px-4 md:px-6 py-3 sm:py-4">
          {/* Search Bar */}
          <div className="relative mb-3 sm:mb-4 text-black">
            <MagnifyingGlassIcon className="w-4 h-4 sm:w-5 sm:h-5 absolute left-2 sm:left-3 top-1/2 transform -translate-y-1/2 text-black" />
            <input
              type="text"
              placeholder="Search records, doctors, or conditions..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-8 sm:pl-10 pr-3 sm:pr-4 py-2 sm:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-black-500 focus:border-transparent text-sm sm:text-base"
            />
          </div>
            ...
            <div className="flex space-x-1 sm:space-x-2 overflow-x-auto pb-2 scrollbar-hide">
              {recordTypes.map((type) => (
                <button
                  ...
                  className={`flex-shrink-0 px-2 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium border whitespace-nowrap ${
      ...
      <div className="px-3 sm:px-4 md:px-6 py-4 sm:py-6">
        {currentView === 'appointments' ? (
          /* Real Medical Records Grid View */
          <>
            <div className="mb-4 sm:mb-6 text-center">
              <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2">Your Medical Records</h2>
              <p className="text-sm sm:text-base text-gray-600">Your complete health records and consultation history</p>
            </div>
            
            {completedAppointments.length === 0 ? (
              <div className="text-center py-8 sm:py-12">
                <DocumentTextIcon className="w-12 h-12 sm:w-16 sm:h-16 text-gray-300 mx-auto mb-3 sm:mb-4" />
                <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-2">No Medical Records Yet</h3>
                <p className="text-sm sm:text-base text-gray-500 mb-4 sm:mb-6 px-4">
                  Your medical records will appear here after doctors add them during consultations.
                </p>
                <button
                  onClick={() => router.push('/book-appointment')}
                  className="px-4 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-cyan-500 to-blue-600 text-white rounded-lg hover:from-cyan-600 hover:to-blue-700 transition-all font-medium text-sm sm:text-base"
                >
                        ...
                        <div className="flex items-center space-x-3">
                          <div className="w-10 h-10 bg-cyan-500 rounded-full flex items-center justify-center">
                            <span className="text-white font-semibold text-sm">
                              {appointment.doctorName ? appointment.doctorName.split(' ')[1]?.charAt(0) || 'D' : 'D'}
                            </span>
                          </div>
                          <div>
                            <h3 className="font-semibold text-gray-900 text-sm">{appointment.doctorName || 'Unknown Doctor'}</h3>
                            <p className="text-xs text-cyan-600">{appointment.doctorSpecialization}</p>
                          </div>
                        </div>
                    ...
                    <div className="p-4">
                      ...
                      <div className="space-y-3 mb-4">
                        ...
                        <div className="flex items-center text-sm text-gray-600">
                          ...
                          <span>{appointment.reason || 'Medical consultation'}</span>
                        </div>
                      ...
                      {appointment.notes && (
                        <div className="bg-green-50 p-3 rounded-lg mb-4">
                          <h4 className="font-medium text-green-900 text-sm mb-1">Examination Notes</h4>
                          <p className="text-green-800 text-xs leading-relaxed">{appointment.notes}</p>
                        </div>
                      )}
                        ...
                        {!appointment.rating ? (
                          ...
                          <div className="text-center">
                            <div className="flex items-center justify-center space-x-1 mb-2">
                              ...
                              <span className="text-xs text-gray-600 ml-1">({appointment.rating}/5)</span>
                            </div>
                            <button
                              onClick={() => openReviewForm(appointment)}
            ...
            <div className="p-6">
              ...
              <div className="bg-gray-50 p-4 rounded-lg mb-6">
                <div className="flex items-center space-x-3 mb-2">
                  ...
                  <div>
                    <h3 className="font-semibold text-black text-sm">{selectedAppointment.doctorName}</h3>
                    <p className="text-xs text-black">{selectedAppointment.doctorSpecialization}</p>
                  </div>
                </div>
                <p className="text-xs text-black">{selectedAppointment.date} at {selectedAppointment.time}</p>
              </div>
              ...
              {/* Review Text */}
                <div className="mb-6">
                  ...
                  <textarea
                    ...
                    className="w-full p-3 border border-gray-300 rounded-lg resize-none focus:ring-2 focus:ring-blue-500 focus:border-transparent placeholder-gray-500 text-black"
                    rows={4}
                    maxLength={500}
                  />
            ...
            <div className="p-6">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-cyan-50 p-4 rounded-lg mb-6 border border-blue-100">
                <div className="text-center mb-4">
                  <h3 className="text-lg font-bold text-black">Medical Prescription</h3>
                  <p className="text-sm text-black">HealthCare Plus Digital Platform</p>
                </div>
                
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <p className="text-black"><strong>Doctor:</strong> {selectedPrescription.doctorName}</p>
                    <p className="text-black"><strong>Specialization:</strong> {selectedPrescription.doctorSpecialization}</p>
                    <p className="text-black"><strong>Date:</strong> {selectedPrescription.date}</p>
                  </div>
                  <div>
                    <p className="text-black"><strong>Time:</strong> {selectedPrescription.time}</p>
                    <p className="text-black"><strong>Consultation:</strong> {selectedPrescription.consultationType}</p>
                    <p className="text-black"><strong>Patient ID:</strong> {selectedPrescription.patientId}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="bg-gray-50 p-3 rounded-lg">
                  <p className="text-sm text-black">{selectedPrescription.diagnosis}</p>
                </div>
              </div>

              {/* Medications */}
              <div className="mb-6">
                <h4 className="font-semibold text-black mb-3">Prescribed Medications</h4>
                <div className="space-y-3">
                  {selectedPrescription.prescriptionData.medications.map((med: any, index: number) => (
                    <div key={index} className="border border-gray-200 rounded-lg p-3">
                      <div className="flex justify-between items-start mb-2">
                        <h5 className="font-medium text-black">{med.name}</h5>
                      ...
                      <p className="text-sm text-black bg-yellow-50 p-2 rounded">
              ...
              <div className="mb-6">
                <h4 className="font-semibold text-black mb-2">General Instructions</h4>
                <div className="bg-green-50 p-3 rounded-lg border border-green-200">
                  <p className="text-sm text-black">{selectedPrescription.prescriptionData.instructions}</p>
                </div>
              </div>

              {/* Restrictions */}
              <div className="mb-6">
                <h4 className="font-semibold text-black mb-2">Restrictions & Warnings</h4>
                <div className="bg-red-50 p-3 rounded-lg border border-red-200">
                  <p className="text-sm text-black">{selectedPrescription.prescriptionData.restrictions}</p>
                </div>
              </div>
