...
export const getAuthData = (): AuthData | null => {
  ...
  try {
    ...
    // Don't modify the user ID - it should be the real UUID from database
    console.log('🔍 Retrieved auth data with UUID:', parsedUserData.id);
...
export const setAuthData = (userData: any, userType: 'patient' | 'doctor', authToken: string): void => {
  ...
  if (!userData || !userType || !authToken) {
    console.warn('Attempted to set invalid auth data:', { userData, userType, authToken });
    return;
  }

  // Don't override existing user ID - use the real UUID from database
  console.log('🔐 Setting auth data with real UUID:', userData.id);
